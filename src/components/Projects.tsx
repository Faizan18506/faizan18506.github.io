...
export default function Projects() {
  ...
  const androidProjects = projectsData.filter(p => p.category === 'android');
  const flutterProjects = projectsData.filter(p => p.category === 'flutter');
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-20">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {androidProjects.map((project, index) => renderProjectCard(project, index))}
          </div>
        </div>

        <div>
          <div className={`mb-10 transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`} style={{ transitionDelay: '0.3s' }}>
            <div className="flex items-center gap-4 mb-8">
              <h3 className="text-3xl font-bold text-white">Flutter Development</h3>
              <div className="flex-1 h-px bg-gradient-to-r from-blue-500/50 to-transparent"></div>
            </div>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {flutterProjects.map((project, index) => (
              renderProjectCard(project, index + androidProjects.length)
            ))}
          </div>
        </div>
      </div>
